...
import { useDeleteFilesMutation, useDownloadFilesMutation, useGetAllFilesQuery } from "@/features/files/filesAPI";
...
const UploadFileLayout = (props: Props) => {
  ...
  const handleCopy = () => {
    const urls = selectedFiles.map((file) =>{
      return `${import.meta.env.VITE_API_URL}/files/${file._id}/view`
    }).join("\n");
    navigator.clipboard
      .writeText(urls)
      .then(() => {
  ...
  downloadFiles(fileIds)
    ...
    .then((res) => {
      ...
      if(res.downloadUrl){
        ...
        link.click();
        link.remove();
        const message=res.isZip? "Zip file is Downloading":"Your file is Downloading"
